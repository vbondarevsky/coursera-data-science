...
coal <- as.character(SCC[coal_index, "SCC"])


# 4. Aggregate Emissions by year
aggNEI <- aggregate(Emissions~year, NEI[NEI$SCC %in% coal,], sum)
...
g <- ggplot(aggNEI, aes(factor(year), Emissions)) + 
    ...
    ggtitle("Total emissions from coal combustion-related sources in the United States 1999-2008")
print(g)
dev.off()
